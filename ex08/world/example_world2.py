...
class My_environment(Environment):
    ...
    def add_static_rectangle_object(self):
        ...
        Returns:
        """
        self.staticRectList.append(['BLACK', pygame.Rect(2,2, self.config['world_width'] - 5, 5),10])
        self.staticRectList.append(['BLACK', pygame.Rect(2, 5, 5, self.config['world_height']-5), 10])
        self.staticRectList.append(['BLACK', pygame.Rect(2, self.config['world_height']-5, self.config['world_width'] - 5,5), 10])
        self.staticRectList.append(['BLACK', pygame.Rect(self.config['world_width'] - 5, 2, 5, self.config['world_height']-5), 10])

        self.staticRectList.append(
            ['BLACK', pygame.Rect(self.config['world_width']/2, self.config['world_height']/5,
                                  5, 4*self.config['world_height']/5 - 10), 5])
        self.staticRectList.append(
            ['BLACK', pygame.Rect(4*self.config['world_width']/5, 2*self.config['world_height']/5,
                                  self.config['world_width']/5-10,5), 5])
        self.staticRectList.append(
            ['BLACK', pygame.Rect(5,3*self.config['world_height']/5, self.config['world_width']/5-10,5 ), 5])
